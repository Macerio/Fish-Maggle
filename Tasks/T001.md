...
1. Mettre le dossier :file_folder:*train* dans le dossier du projet. Attention à ne pas commit le dossier train :satisfied: (Normalement  *train* et *test_stg1* sont dans le .gitignore pour qu'on ne nous propose pas de commit pas ces dossiers)  

1. Vérifier que toutes les commandes marchent jusqu'à la ligne avant   
```{r}
ui <- bootstrapPage(
```

1. Compiler la fonction **ui** et la fonction **server**.  

1. Normalement l'application devrait se lancer en compilant la dernière ligne :  
```{r}
